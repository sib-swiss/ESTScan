...
#
# Copyright (c) 1999-2002 Swiss Institute of Bioinformatics. All rights reserved.
#
################################################################################
...
sub readConfig {
    ...
    my($parFileName, $forcedtuplesize, $forcedminmask, $forcedpseudocounts, 
       $forcedminscore, $forcedstartlength, $forcedstartpreroll, $forcedstoplength, 
       $forcedstoppreroll, $verbose) = @_;

    my($organism, $dbfiles, $ugdata, $estdata, $datadir, $filestem, $rnafile, $estfile, 
       $estcdsfile, $estutrfile,  $trainingfile, $testfile, $utrfile, $cdsfile, 
       $tuplesize, $minmask, $pseudocounts, $minscore, $startlength, $startpreroll, 
       $stoplength, $stoppreroll, $smatfile, $estscanparams, $nb_isochores);
    ...
    if (!defined($startlength)) { $startlength = $startpreroll + ceil($tuplesize/3); }
    if (!defined($stoplength))  { $stoplength  = $stoppreroll  + ceil($tuplesize/3); }
    ...
    if (!defined($rnafile))      { $rnafile      = "$datadir/mrna.seq";                }
    if (!defined($estfile))      { $estfile      = "$datadir/ests.seq";                }
    if (!defined($estcdsfile))   { $estcdsfile   = "$datadir/Evaluate/estcds.seq";     }
    if (!defined($estutrfile))   { $estutrfile   = "$datadir/Evaluate/estutr.seq";     }
    if (!defined($trainingfile)) { $trainingfile = "$datadir/training.seq";            }
    if (!defined($testfile))     { $testfile     = "$datadir/test.seq";                }
    if (!defined($utrfile))      { $utrfile      = "$datadir/Evaluate/rnautr.seq";     }
    if (!defined($cdsfile))      { $cdsfile      = "$datadir/Evaluate/rnacds.seq";     }
    if (!defined($smatfile))     { $smatfile     = "$datadir/Matrices/$filestem.smat"; }


    # check if parameters are plausible
    if ($startpreroll >= $startlength) { 
	die("build_tablse: preroll in start profile too large " . 
	    "($startpreroll >= $startlength)");
    }
    if ($stoppreroll > $stoplength) { 
	die("build_tablse: preroll in stop profile too large " . 
	    "($stoppreroll >= $stoplength)");
    }
    if ($tuplesize > 11) { die "build_tables: analysed tuples too big ($tuplesize)"; }

    return($organism, $dbfiles, $ugdata, $estdata, $datadir, $filestem,
...
sub showConfig {
    ...
    my($parFileName, $organism, $dbfiles, $ugdata, $estdata, $datadir, 
       $rnafile, $estfile, $estcdsfile, $estutrfile, $trainingfile, $testfile, 
       $utrfile, $cdsfile, $tuplesize, $minmask, $pseudocounts, $minscore, 
       $startlength, $startpreroll, $stoplength, $stoppreroll, $smatfile, 
    ...
    if ($nb_isochores>0) { log_print(" - nb of isochores:              $nb_isochores"); }
    else {
	my @isos;
...
sub wrapped_log_print {
    ...
    foreach (@array) {
	$_ .= ', ';
	if (($linelen + length($_)) < $width) { $buffer .= $_; $linelen += length($_); }
	else { $buffer .= "\n$filler$_"; $linelen = length("$filler$_"); }
    }
