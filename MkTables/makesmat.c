...
static void usage(char *arg0)
{
  fprintf(stderr, "Usage: %s [options] < infile > outfile\n"
	  "  where options are:\n"
	  "    -t <int>   size of redundancy filter\n"
	  "    -p <int>   pseudocounts to be added\n"
	  "    -f <int>   number of frames in start profiles\n"
	  "    -o <int>   site offset within start profiles\n"
	  "    -F <int>   number of frames in stop profiles\n"
	  "    -O <int>   site offset within stop profiles\n"
	  "    -T <s|p|c> output type: scores, probabilities or counts\n"
	  "    -m <int>   minimum score\n"
	  "    -d         debug\n",
	  arg0);
  exit(1);
...
static void getOptions(int argc, char *argv[])
{
  ...
  /* read command line */
  while (1) {
    int c = getopt(argc, argv, "t:p:f:F:o:O:T:m:dh");
    if (c == -1) break;
    switch (c) {
  ...
  if ((options.startFrames - options.startOffset + 1)< options.tupsize) {
    fprintf(stderr, "makesmat: start profile, tuple size to large (%d-%d>=%d)\n", 
  ...
  if ((options.output_type != 's') && (options.output_type != 'p') &&
...
{
  int dfloor = floor(d);
  return ((d - dfloor) < 0.5) ? dfloor : (dfloor+1);
}
...
static void print_tuple(unsigned long index, int len)
{
  int i;
  char *t = (char *)alloca(sizeof(char) * len);
...
static void initCounters() 
{
  ...
  int nbTuples1 = (1 << (2*(options.tupsize - 1)));
   ...
   for (i = 0; i < 4; i++) {
     ...
     ctr[i].tupsize1Ctr = (unsigned long *)malloc(sizeof(unsigned long) * nbTuples1);
     ctr[i].tupsizeCtr  = (unsigned long *)malloc(sizeof(unsigned long) * nbTuples);
     memset(ctr[i].tupsize1Ctr, 0, sizeof(unsigned long) * nbTuples1);
     memset(ctr[i].tupsizeCtr,  0, sizeof(unsigned long) * nbTuples);

     startctr[i] = (unsigned long *)malloc(sizeof(unsigned long) * options.startFrames);
     memset(startctr[i], 0, sizeof(unsigned long) * options.startFrames);
     stopctr[i] = (unsigned long *)malloc(sizeof(unsigned long) * options.stopFrames);
...
static void update_counters(unsigned long index, int frame, int skip) 
...
void count_tuples()
{
  ...
  while(c == '>') {
    ...
    s += 5;  cdsStart = atoi(s) - 1; /* C index start at 0! */
    s = strchr(s, ' ') + 1;        cdsEnd =   atoi(s) - 1; /* C index start at 0! */
	...
	longbuf = realloc(longbuf, sizeof(char) * size); 
      ...
      longbuf[pos] = getCode(c); pos++;
      c = get_nucleotide();
    }

     /* count single nucleotides */
    for (i = 0; i < pos; i++) {
      if (longbuf[i] < N) { singleCtr[longbuf[i]]++; singleTotal++; }
    }
    ...
    for (i = 0; i <= cdsStart - options.startOffset; i++) {
      if (longbuf[i] < N) tupleIndex = ((tupleIndex<<2) | longbuf[i]) & tupleIndexMask;
      else skip = options.tupsize; 
      if (skip) skip--;
    ...
    while (i < j + options.startFrames) {
       if (longbuf[i] < N) startctr[longbuf[i]][i - j]++; 
       i++;
     }
     ...
     for (i = cdsStart + options.startFrames - options.startOffset + 1; 
	  i <= cdsEnd - options.stopOffset; i++) {
       if (longbuf[i] < N) tupleIndex = ((tupleIndex<<2) | longbuf[i]) & tupleIndexMask;
       else skip = options.tupsize; 
       if (skip) skip--;
       update_counters(tupleIndex, (i - cdsStart)%3, skip); /* update UTR counters */
     }

     /* count stop profile */
     for (i = cdsEnd - options.stopOffset + 1; 
	  (i <= cdsEnd + options.stopFrames - options.stopOffset) && (i < pos); i++) {
       if (longbuf[i] < N) stopctr[longbuf[i]][i - cdsEnd + options.stopOffset - 1]++;
     }

     /* count 3'UTR */
     tupleIndex = 0;
     skip = options.tupsize;   /* tupleIndex is invalid for the next skip positions */
     for (i = cdsEnd + options.stopFrames - options.stopOffset + 1; i < pos; i++) {
       if (longbuf[i] < N) tupleIndex = ((tupleIndex<<2) | longbuf[i]) & tupleIndexMask; 
       else skip = options.tupsize; 
       if (skip) skip--;
       update_counters(tupleIndex, 3, skip); /* update UTR counters */
...
static double pseudo(int tuple, int tupsize)
{
  int i;
  double p = 1.0;
  for (i = 0; i < tupsize; i++) {
...
static void print_cdstable(double *singleProb) 
 {
   int f, i, *scores[3];
   double *tuple1Prob[3], *tupleProb[3], currTotal, score;
   char typec;
   ...
   for (f = 0; f < 3; f++) {
     if (options.debug) printf("Probabilities for frame %d\n", f, options.tupsize - 1);
     tuple1Prob[f] = (double *)malloc(sizeof(double) * nbTuples1);
     for (i = 0; i < nbTuples1; i++) {
       tuple1Prob[f][i] = ((double)ctr[f].tupsize1Ctr[i]+pseudo(i, options.tupsize-1)) / 
	                   (double)(ctr[f].tupsize1Total + options.pseudocounts); 
       if (options.debug) {
	 print_tuple(i, options.tupsize-1);
	 printf(":%8g=(%8d + %8g)/(%8d + %8d)\n",tuple1Prob[f][i],ctr[f].tupsize1Ctr[i],
		pseudo(i, options.tupsize-1),ctr[f].tupsize1Total,options.pseudocounts);
       }
     }
     if (options.debug) printf("Probabilities for frame %d\n", f, options.tupsize);
     tupleProb[f] = (double *)malloc(sizeof(double) * nbTuples);
     for (i = 0; i < nbTuples; i++) {
       tupleProb[f][i] = ((double)ctr[f].tupsizeCtr[i]+pseudo(i, options.tupsize)) / 
	                 (double)(ctr[f].tupsizeTotal + options.pseudocounts); 
       if (options.debug) {
	 print_tuple(i, options.tupsize);
	 printf(":%8g=(%8d + %8g) / (%8d + %8d)\n",tupleProb[f][i], ctr[f].tupsizeCtr[i],
		pseudo(i, options.tupsize), ctr[f].tupsizeTotal, options.pseudocounts);
       }
     }
   }
   ...
   for (f = 0; f < 3; f++) { 
     if (options.debug) printf("Log-odds for frame %d\n", f);
     scores[f] = (int *)malloc(sizeof(int) * nbTuples);
     for (i = 0; i < nbTuples; i++) {
       if ((tupleProb[f][i] == 0.0) || (tuple1Prob[f][i >> 2] == 0.0)) 
	 scores[f][i] = options.minscore;
       else {
	 score=log(tupleProb[f][i]/singleProb[i&3]/tuple1Prob[f][i>>2])/M_LN2*10.0;
	 scores[f][i] = (score < options.minscore) ? options.minscore : round(score);
       }
       if (options.debug) {
	 print_tuple(i, options.tupsize);
	 printf(": %8g = %8g / %8g, %4d = 10*log(%8g / %8g)\n", 
		...
		tupleProb[f][i],  tuple1Prob[f][i>>2],
		scores[f][i], tupleProb[f][i]/tuple1Prob[f][i>>2], singleProb[i&3]);
       }
     }
   }
   ...
   if (options.debug) { 
     if (options.output_type == 'p') 
       printf("single: %.6f %.6f %.6f %.6f\n", singleProb[0], singleProb[1], 
	      singleProb[2], singleProb[3]);
     if (options.output_type == 'c') 
       printf("single (total %d): %-6d %-6d %-6d %-6d\n", singleTotal,
	      singleCtr[0], singleCtr[1], singleCtr[2], singleCtr[3]);
   }
   for (f = 0; f < 3; f++) {
	 ...
	 if (options.output_type == 'p') printf(" (total %.5f)", tuple1Prob[f][i >> 2]); 
	 if (options.output_type == 'c') printf(" (total %d)",ctr[f].tupsize1Ctr[i>>2]);
	 printf(": ");
       }
       switch (options.output_type) {
       case 's': 
	 printf("%-6d %-6d %-6d %-6d\n", scores[f][i], scores[f][i+1], 
		scores[f][i+2], scores[f][i+3]);
	 break;
       case 'p': 
	 printf("%.6f %.6f %.6f %.6f\n", tupleProb[f][i], tupleProb[f][i+1], 
		tupleProb[f][i+2], tupleProb[f][i+3]);
	 break;
       case 'c': 
	 printf("%-6d %-6d %-6d %-6d\n", ctr[f].tupsizeCtr[i], ctr[f].tupsizeCtr[i+1], 
		ctr[f].tupsizeCtr[i+2], ctr[f].tupsizeCtr[i+3]);
       }
     }
   }
...
static void print_utrtable(double *singleProb)
{
  int i, *scores;
  double *tuple1Prob, *tupleProb, currTotal, score;
  char typec;
  ...
  for (i = 0; i < nbTuples1; i++) 
    tuple1Prob[i] = ((double)ctr[3].tupsize1Ctr[i]+pseudo(i, options.tupsize-1)) / 
                    (currTotal + options.pseudocounts); 
  currTotal = ctr[3].tupsizeTotal;
  tupleProb = (double *)malloc(sizeof(double) * nbTuples);
  for (i = 0; i < nbTuples; i++) 
    tupleProb[i] = ((double)ctr[3].tupsizeCtr[i]+pseudo(i, options.tupsize)) / 
                   (currTotal + options.pseudocounts); 

  /* compute log-odds and scores */
  scores = (int *)malloc(sizeof(int) * nbTuples);
  for (i = 0; i < nbTuples; i++) {
    if ((tupleProb[i] == 0.0) || (tuple1Prob[i>>2] == 0.0)) scores[i] = options.minscore;
    else {
      score = log(tupleProb[i] / singleProb[i&3] / tuple1Prob[i>>2]) / M_LN2 * 10.0;
  ...
  for (i = 0; i < 1 << (2*options.tupsize); i += 4) {
    ...
    if (options.debug) { 
      print_tuple(rowIndex, options.tupsize-1); 
      if (options.output_type == 'p') printf(" (total %.5f)", tuple1Prob[i >> 2]); 
      if (options.output_type == 'c') printf(" (total %d)", ctr[3].tupsize1Ctr[i>>2]); 
      printf(": ");
    }
    switch (options.output_type) {
    case 's': 
      ...
      break;
    case 'p': 
      printf("%.6f %.6f %.6f %.6f\n", tupleProb[i], tupleProb[i+1], 
	     tupleProb[i+2], tupleProb[i+3]);
      break;
    case 'c': 
      printf("%-6d %-6d %-6d %-6d\n", ctr[3].tupsizeCtr[i], ctr[3].tupsizeCtr[i+1], 
	     ctr[3].tupsizeCtr[i+2], ctr[3].tupsizeCtr[i+3]);
    }
  }
...
static void print_pssm(double *singleProb, unsigned long **ctr, int frames)
{
  int i, j;
  for (i = 0; i < frames; i++) {
    int currTotal = 0;
    for (j = 0; j < 4; j++) { currTotal += ctr[j][i]; }
    for (j = 0; j < 4; j++) {
      double pseudo = options.pseudocounts*(double)singleCtr[j]/(double)singleTotal;
      double p = ((double)ctr[j][i]+pseudo) / ((double)currTotal + options.pseudocounts);
      switch(options.output_type) {
      case 'c': printf("%-6d ", ctr[j][i]); break;
      case 'p': printf("%.6f ", p); break;
      case 's': 
	if ((p == 0.0) || (singleProb[j] == 0.0)) printf("%-6d ", options.minscore);
	else {
	  double score = log(p / singleProb[j]) / M_LN2 * 10.0;
	  printf("%-6d ", (score<options.minscore) ? options.minscore : round(score));
	}
      }
    }
    printf("\n");
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < 4; i++) singleProb[i] = (double)singleCtr[i] / currTotal; 
